...
COMMON_ATTRS = {
    ...
    "resource_logical_names": attr.string_dict(
        doc = "A mapping of resource file basenames to their explicit manifest resource names. " +
              "This matches MSBuild's EmbeddedResource LogicalName metadata. When a resource " +
              "file listed in 'resources' has its basename in this dict, the specified name is " +
              "used as the manifest resource name instead of the default computed name.",
    ),
    "out": attr.string(
...
CSHARP_COMMON_ATTRS = dicts.add(
    COMMON_ATTRS,
    {
        ...
        "use_shared_compilation": attr.bool(
            doc = "When True, uses the Roslyn compiler server (VBCSCompiler) via a " +
                  "Bazel persistent worker, avoiding repeated JIT startup costs. " +
                  "Equivalent to the .NET SDK's UseSharedCompilation property.",
            default = False,
        ),
        "shared_compilation_worker": attr.label(
            doc = "Internal: the persistent worker binary for shared compilation. Do not set directly.",
            default = None,
            executable = True,
            cfg = default_transition,
        ),
